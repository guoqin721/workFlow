<template>
  <div class="process-panel__container" :style="{ width: `${width}px` }">
    <el-collapse v-model="activeTab" accordion>
      ...
      <el-collapse-item v-if="elementType === 'Process'" key="message" name="message">
      ...
      <el-collapse-item v-if="formVisible" key="form" name="form">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-order" />任务表单</div>
        <element-form :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="formVisible" key="variable" name="variable">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-data" />任务变量</div>
        <form-variable :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="elementType && elementType.indexOf('Task') !== -1" key="task" name="task">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-claim" />任务设置</div>
        <element-task :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="elementType && elementType.indexOf('Task') !== -1" key="multiInstance" name="multiInstance">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-help" />多实例</div>
        <element-multi-instance :business-object="elementBusinessObject" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="elementType" key="listeners" name="listeners">
      ...
      <el-collapse-item v-if="elementType" key="extensions" name="extensions">
        <div slot="title" class="panel-tab__title"><i class="el-icon-circle-plus" />扩展属性</div>
        <element-properties :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item v-if="elementType" key="other" name="other">
